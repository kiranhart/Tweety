...
import ca.tweetzy.tweety.exception.InvalidCommandArgException;
		...
		try {
			...
			if (args.length < getMinArguments() || autoHandleHelp && args.length == 1 && ("help".equals(args[0]) || "?".equals(args[0]))) {
				...
				Common.runAsync(() -> {
					...
					paginator.setTweetyHeader(SimpleLocalization.Commands.LABEL_HELP_FOR.replace("{label}", getLabel() + sublabel)).setPages(Common.toArray(pages));
