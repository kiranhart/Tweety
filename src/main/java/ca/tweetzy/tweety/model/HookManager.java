...
public final class HookManager {
	...
	public static boolean isIgnoring(final UUID player, final UUID who) {
		...
		return isEssentialsLoaded() ? essentialsHook.isIgnoring(player, who) : isCMILoaded() && CMIHook.isIgnoring(player, who);
	}
	...
	public static boolean isChatIntegrated() {
		return isVaultLoaded() && vaultHook.isChatIntegrated();
	}
	...
	public static boolean isEconomyIntegrated() {
		return isVaultLoaded() && vaultHook.isEconomyIntegrated();
	}
	...
	public static boolean isNPC(final Entity entity) {
		return isCitizensLoaded() && citizensHook.isNPC(entity);
	}
...
class VaultHook {
	...
	Boolean hasPerm(@NonNull final OfflinePlayer player, final String perm) {
		try {
			return permissions != null ? perm == null || permissions.playerHas(null, player, perm) : null;
	...
	Boolean hasPerm(@NonNull final String player, final String perm) {
		return permissions != null ? perm == null || permissions.has((String) null, player, perm) : null;
	}

	Boolean hasPerm(@NonNull final String world, @NonNull final String player, final String perm) {
		return permissions != null ? perm == null || permissions.has(world, player, perm) : null;
	}
		...
		for (final World w : Bukkit.getWorlds()) {
			...
			if (legacy)
				try {
					...
					throw new TweetyException("Failed WorldEdit 6 legacy hook, see above & report");
				}
			else
	...
	private Object getRegionManager(final World w) {
		if (legacy)
			try {
				...
				throw new TweetyException("Failed WorldGuard 6 legacy hook, see above & report");
			}
		...
		try {
			...
			throw new TweetyException("Failed WorldGuard hook, see above & report");
		}
	}
}
		...
		try {
			...
			try {
				...
				try {
					...
					throw new TweetyException(ex3, "PlotSquared could not convert " + player.getName() + " into PlotPlayer! Is the integration outdated?");
				}
			}
		}
		...
		try {
			...
			if (mob != null) {
				...
				if (mythicEntity != null)
					return ReflectionUtil.invoke("getName", mythicEntity);
			}
