...
import ca.tweetzy.tweety.bungee.SimpleBungee;
...
import ca.tweetzy.tweety.menu.MenuListener;
...
public abstract class SimplePlugin extends JavaPlugin implements Listener {
	...
	public final void onEnable() {
		...
		try {
			...
			TweetyPacketListener.addNativeListener();
	...
	protected final void registerCommands(final StrictList<String> labelAndAliases, final SimpleCommandGroup group) {
		...
		if (getMainCommand() != null && getMainCommand().getLabel().equals(labelAndAliases.get(0)))
			throw new TweetyException("Your main command group is registered automatically!");
	...
	private TweetyException unsupported(final String method) {
		return new TweetyException("Cannot call " + method + " in " + getName() + ", use YamlConfig or SimpleCommand classes in Foundation for that!");
	}
