...
public class ChatInternals {
		...
		if (MinecraftVersion.newerThan(MinecraftVersion.V.v1_6) && MinecraftVersion.olderThan(MinecraftVersion.V.v1_12))
			try {
				...
				if (MinecraftVersion.newerThan(MinecraftVersion.V.v1_11))
					...
					chatMessageConstructor = MinecraftVersion.newerThan(MinecraftVersion.V.v1_7) ? chatPacket.getConstructor(chatBaseComponent, byte.class) : chatPacket.getConstructor(chatBaseComponent);
				...
				if (MinecraftVersion.olderThan(MinecraftVersion.V.v1_8))
					...
					throw new ReflectionUtil.ReflectionException(ex, "Error initiating Chat/Title/ActionBAR API (incompatible Craftbukkit? - " + Bukkit.getVersion() + " / " + Bukkit.getBukkitVersion() + " / " + MinecraftVersion.getServerVersion() + ")");
				}
			}
	}
	...
	public static void sendTitleLegacy(final Player player, final int fadeIn, final int stay, final int fadeOut, final String title, final String subtitle) {
		Valid.checkBoolean(MinecraftVersion.olderThan(MinecraftVersion.V.v1_12), "This method is unsupported on MC 1.13 and later");
		...
		try {
			...
			throw new ReflectionUtil.ReflectionException(ex, "Error sending title to: " + player.getName() + ", title: " + title + ", subtitle: " + subtitle);
		}
	}
