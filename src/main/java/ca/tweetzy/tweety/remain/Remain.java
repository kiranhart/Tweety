...
import ca.tweetzy.tweety.remain.nbt.NBTInternals;
...
public final class Remain {
	...
	public static void unregisterCommand(final String label, final boolean removeAliases) {
		try {
			...
			throw new TweetyException(ex, "Failed to unregister command /" + label);
		}
	}
	...
	public static SimpleCommandMap getCommandMap() {
		try {
			...
			throw new TweetyException(ex, "Unable to get the command map");
		}
	}
	...
	public static String getLocale(final Player player) {
		try {
			...
			try {
				...
				return ReflectionUtil.invoke(method, spigot);
		...
		Common.runLater(delayTicks, () -> {
			try {
				...
				try {
					...
					throw new TweetyException(e, "Failed to send respawn packet to " + player.getName());
				}
			}
		});
	}
	...
	public static void setPages(BookMeta meta, List<BaseComponent[]> pages) {
		try {
			...
			try {
				final List<Object> chatComponentPages = ReflectionUtil.getFieldContent(ReflectionUtil.getOBCClass("inventory.CraftMetaBook"), "pages", meta);
	...
	public static String getName(final Entity entity) {
		try {
			...
			return entity instanceof Player ? entity.getName() : ItemUtil.bountifyCapitalized(entity.getType());
		}
	}
	...
	public static boolean isInvisible(Entity entity) {
		...
		if (entity instanceof LivingEntity && MinecraftVersion.atLeast(V.v1_16))
			...
			return ReflectionUtil.invoke("isInvisible", nmsEntity);
		}
	}
	...
	public static Block getTargetBlock(final LivingEntity en, final int radius) {
		try {
			return en.getTargetBlock(null, radius);
			...
			try {
				return (Block) en.getClass().getMethod("getTargetBlock", HashSet.class, int.class).invoke(en, null, radius);

			} catch (final ReflectiveOperationException ex2) {
				throw new TweetyException(t, "Unable to get target block for " + en);
			}
		}
	}
	...
	public static boolean isStatSavingDisabled() {
		try {
			...
			try {
				...
				return cfg.isSet("stats.disable-saving") && cfg.getBoolean("stats.disable-saving");
			} catch (final Throwable t) {
				// No Spigot
			}
		}
	...
	public static void sneaky(final Throwable throwable) {
		try {
			...
			throw new TweetyException(throwable);
		}
	}
	...
	private static Player[] getPlayersLegacy() {
		try {
			...
			throw new TweetyException(ex, "Reflection malfunction");
		}
	}
	...
	private static void sendComponent0(final CommandSender sender, final BaseComponent... comps) {
		...
		try {
			...
			if (MinecraftVersion.atLeast(V.v1_7) && !Bukkit.getName().contains("Cauldron"))
				Common.throwError(ex, "Failed to send component: " + plainMessage + " to " + sender.getName());
