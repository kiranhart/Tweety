...
public abstract class YamlConfig {
	...
	private void saveWithoutSerializing() {
		instance.save(header != null ? header : getFileName().equals(TweetyConstants.File.DATA) ? TweetyConstants.Header.DATA_FILE : TweetyConstants.Header.UPDATED_FILE);
	}
	...
	protected final String getString(final String path) {
		...
		throw new TweetyException("Excepted string at '" + path + "' in " + getFileName() + ", got (" + object.getClass() + "): " + object);
	}
	...
	protected final <T> List<T> getList(final String path, final Class<T> type, final Object... deserializeParameters) {
		...
		if (type == Map.class && deserializeParameters != null & deserializeParameters.length > 0 && deserializeParameters[0] != String.class)
			throw new TweetyException("getList('" + path + "') that returns Map must have String.class as key, not " + deserializeParameters[0]);
	...
	protected final String[] getStringArray(final String path) {
		...
		throw new TweetyException("Excepted string or string list at '" + path + "' in " + getFileName() + ", got (" + array.getClass() + "): " + array);
	}
	...
	protected final List<String> getStringList(final String path) {
		...
		throw new TweetyException("Excepted a list at '" + path + "' in " + getFileName() + ", got (" + raw.getClass() + "): " + raw);
	}
	...
	private void forceSingleDefaults(final String path) {
		if (useDefaults && getDefaults() != null)
			throw new TweetyException("Cannot use get method with default when getting " + formPathPrefix(path) + " and using a default config for " + getFileName());
	}
	...
	private void checkAssignable(final boolean fromDefault, final String path, final Object value, final Class<?> clazz) {
		if (checkAssignables && !clazz.isAssignableFrom(value.getClass()) && !clazz.getSimpleName().equals(value.getClass().getSimpleName())) {
			...
			throw new TweetyException("Malformed configuration! Key '" + path + "' in " + (fromDefault ? "inbuilt " : "") + getFileName() + " must be " + clazz.getSimpleName() + " but got " + value.getClass().getSimpleName() + ": '" + value + "'");
		}
	}
	...
	public final class CasusHelper {
		...
		private CasusHelper(final String raw) {
			...
			if (values.length != 3)
				throw new TweetyException("Malformed type, use format: 'second, seconds' OR 'sekundu, sekundy, sekund' (if your language has it)");
